...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  Stream<HomeState> mapEventToState(HomeEvent event) async* {
    ...
    if (event is SendPolicy) {
      yield* _mapSendPolicyToState(event);
    }
    if (event is SavePolicy) {
      yield* _mapSavePolicyToState(event);
    }
  ...
  Stream<HomeState> _mapSavePolicyToState(SavePolicy event) async* {
    yield InitialState();
    try {
      var response = await reporterRepository.checkServiceStatus();
      if (response.statusCode == 200) {
        String urlReport = reporterRepository.sendPolicy(event.client);
        yield ReportReady(urlReport);
      } else if (response.statusCode == 500) {
        yield const ShowNotification("servicio no disponible");
      } else {
  ...
  Stream<HomeState> _mapSendPolicyToState(SendPolicy event) async* {
    yield InitialState();
    try {
      var response = await reporterRepository.checkServiceStatus();
      if (response.statusCode == 200) {
        var notifyResponse = await notifierRepository.sendNotification('/policies/policy.pdf');
        if (notifyResponse.statusCode == 200) {
          yield const ShowNotification("Póliza enviada");
        } else {
          yield const ShowNotification("Error en el envío de póliza");
        }
      } else if (response.statusCode == 500) {
        yield const ShowNotification("servicio no disponible");
      } else {
